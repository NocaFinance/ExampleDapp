...
//       //   });
//       case NetworkId.ETHEREUM_TESTNET:
//         return this._get({
...
//           fallback: "https://data-seed-prebsc-2-s2.binance.org:8545/",
//         });
...
export function getNetworkIdFromChainId(chainId: number) {
  console.log("New network", chainId);
  const stringNetworkId = Object.keys(evmChainId).find(
    (key) => evmChainId[parseInt(key) as NetworkId] == chainId
  );
  console.log("New network", stringNetworkId);
  return stringNetworkId ? (parseInt(stringNetworkId) as NetworkId) : -1;
}
...
export class Providers {
  ...
  public static getProviderUrl(networkId: NetworkId) {
    const url = NETWORKS[networkId].rpcUrls[0];
    return url;
  }
